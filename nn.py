...
class Graph(object):
    ...
    def add(self, node):
        ...
        inputs = [self.outputs[parent] for parent in parents]
    ...
    def backprop(self):
        ...
        for node in reversed(self.get_nodes()):
            backwards = node.backward(self.get_inputs(node), self.get_gradient(node))
            for back, parent in zip(backwards, node.get_parents()):
                self.gradients[parent] = self.gradients[parent] + back
    ...
    def step(self, step_size):
        ...
        for node in self.nodes:
            if isinstance(node, Variable):
                node.data = node.data - self.gradients[node]*step_size
...
class ReLU(FunctionNode):
    ...
    def forward(inputs):
        "*** YOUR CODE HERE ***"
        return np.maximum(inputs[0], np.zeros(np.shape(inputs[0])))

    @staticmethod
    def backward(inputs, gradient):
        "*** YOUR CODE HERE ***"
        return [np.multiply((inputs[0] >= 0).astype(int), gradient)]
...
class SquareLoss(FunctionNode):
    ...
    def forward(inputs):
        "*** YOUR CODE HERE ***"
        return np.mean(.5 * np.square(np.subtract(inputs[0], inputs[1])))

    @staticmethod
    def backward(inputs, gradient):
        "*** YOUR CODE HERE ***"
        newGradient = gradient * np.subtract(inputs[0], inputs[1]) / inputs[0].size
